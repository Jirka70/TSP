...
class PlotlyVisualizer(IVisualizer):
    ...
    def __init__(self, config: VisualizationConfig) -> None:
        ...
        if not PLOTLY_AVAILABLE:
            log.error("Plotly is not installed. Please install it using 'pip install plotly'.")

    def visualize_raw(self, data: RawPreprocessedDTO, run_ctx: RunContext) -> None:
        """Visualizes PSD of the first recording using Plotly."""
        if not self._config.visualize_raw or not data.data or not PLOTLY_AVAILABLE:
            return
        ...
        if hasattr(raw, "compute_psd"):
            ...
            fig.update_layout(
                title=f"Power Spectral Density - Subject {recording.subject_id}",
    ...
    def visualize_epochs(self, data: EpochPreprocessedDTO, run_ctx: RunContext) -> None:
        ...
        if not self._config.visualize_epochs or not data.data or not PLOTLY_AVAILABLE:
            return
        ...
        if hasattr(epochs, "get_data"):
            ...
            fig.update_layout(
                title=f"ERP Average - Subject {recording.subject_id}",
                xaxis_title="Time (s)",
    ...
    def visualize_augmentation(self, data: DatasetSplitDTO, run_ctx: RunContext) -> None:
        ...
        if not self._config.visualize_augmentation or not data.folds or not PLOTLY_AVAILABLE:
            return
        ...
        if isinstance(x, np.ndarray) and x.ndim == 3:
            ...
            for i in range(n_samples):
                fig.add_trace(
                    go.Scatter(y=x[i, 0, :], mode='lines', name=f'Sample {i}'),
                    row=i+1, col=1
                )
    ...
    def visualize_evaluation(self, data: EvaluationResultDTO, run_ctx: RunContext, model_name: str) -> None:
        ...
        if not self._config.visualize_evaluation or not PLOTLY_AVAILABLE:
            return
        ...
        metrics_df = pd.DataFrame([
            {"Metric": k, "Value": v} for k, v in data.metrics.items()
        ])
        fig_metrics = px.bar(
            metrics_df, 
            x="Metric", 
    ...
    def _handle_output(self, fig: object, filename: str) -> None:
        ...
        if not self._config.save_plots or not PLOTLY_AVAILABLE:
            return
